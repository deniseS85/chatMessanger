    ...
    db.query(checkUserQuery, [name, phoneNumber], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            db.query(checkRequestQuery, [userId, contactId], (err, requestResults) => {
                if (err) {
                    return res.status(500).json({ message: 'Error checking the request' });
                }
                ...
                if (requestResults.length === 0) {
                    ...
                    db.query(addRequestQuery, [userId, contactId], (err) => {
                        if (err) {
                            return res.status(500).json({ message: 'Error sending the contact request' });
                        }

                        return res.status(200).json({ message: 'Send contact request', name });
                    });
                } else {
